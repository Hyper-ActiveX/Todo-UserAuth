...
function TodoForm( {fetchUserTodos, BASE_URL} ) {
  ...
  return (
    <>
    <div className='flex w-full h-[3rem] bg-[black] justify-between content-center px-8'>
      <div className='text-[white] pt-2 text-xl font-serif'>Wellcome to your todo list</div>
      <div>
        <button onClick={() => navigate(`/`)} type='' className='text-[#242B2E] bg-[#CAD5E2] rounded-[0.3rem] px-3 py-2'>Logout</button>
      </div>
    </div>
      <div className='flex flex-col items-center justify-center mt-[10rem]'>
       
       <form  onSubmit={handleTitleSubmit}   className='todoForm flex flex-col items-center justify-center  w-[50rem] h-[10rem] rounded-[1rem] bg-[black] mb-[3rem] '>
