...
function TodoLists({ fetchUserTodos, userTodos, setUserTodos, BASE_URL }) {
  ...
  return (
    ...
    <div>
      ...
      <form className='todoForm flex flex-row items-center justify-between border-sold border-2 w-[50rem] h-[5rem] rounded-[1rem] bg-[black] mb-[1rem]  ' onSubmit={handleSearch} >
        <div>
      ...
      <div className='shadow-md bg-slate-100/50  w-[50rem] mx-auto mt-[1rem] rounded-[1rem] bg-[black]'>


        {userTodos && userTodos.map((user) => {

          return <div className='overflow-hidden rounded-[1rem]'>
            {/* title bar */}

            <label>
              <input className='opacity-0 peer' type="checkbox" />
              <div className='flex flex-row items-center justify-between  mx-[1rem] rounded-[0.5rem] bg-indigo-200'>
                <div className='flex flex-row items-center justify-between text-[black] 	font-semibold	'>
